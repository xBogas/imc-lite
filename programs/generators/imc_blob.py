...
hpp.append(comment('Retrieve pointer to blob data.'))
hpp.append(comment('@return pointer to blob data.'))
f = Function('getData', 'const unsigned char*', static=True)
hpp.append(f.decl());

hpp.append(comment('Retrieve size of the blob in bytes.'))
hpp.append(comment('@return blob\'s size.'))
