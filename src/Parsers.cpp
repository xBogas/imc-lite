...
#include "Factory.def"
	...
	static uint16_t 
	compute_CRC16(const uint8_t* bfr, uint16_t bfr_len)
	{
		uint16_t crc = 0;
		while (bfr_len--)
	...
	static void
	parserHeader(Header& hdr, const uint8_t* msg)
	{
		uint64_t data;
	...
	static Message*
	parserPayload(const Header& hdr, const uint8_t* bfr)
	{
		// Received CRC
		uint16_t r_crc = 0;
	...
	static uint16_t 
	serializeHeader(const Message* msg, uint8_t* bfr)
	{
		uint8_t* ptr = bfr;
